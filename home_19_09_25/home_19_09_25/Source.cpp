...
int main() {
	...
	const char* days[7] = {
			"Ïîíåäåëüíèê","Âòîðíèê","Ñðåäà",
			"×åòâåðã","Ïÿòíèöà","Ñóááîòà","Âîñêðåñåíüå"
	};

	int spends[7];
	cout << "Ââåäèòå ðàñõîäû çà íåäåëþ (â äîëëàðàõ):\n";
	for (int i = 0; i < 7; i++)
	{
		cout << days[i];
		cin >> spends[i];
	...
	cout << "\nÎáùàÿ ñóììà: " << AllSpend(spends, 7) << " $\n";
	cout << "Ñðåäíÿÿ ñóììà: " << MidSpend(spends, 7) << " $\n";

	int n;
	SpendDay* big = More100d(spends, 7, n);
	if (n > 0) {
		cout << "Äíè ñ ðàñõîäàìè >100$:\n";
		for (int i = 0; i < n; i++)
